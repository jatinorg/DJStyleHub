    ...
    <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-neutral-200">
      ...
      <div className="bg-neutral-900 text-neutral-300 text-xs py-1.5 px-4">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <p className="tracking-wide">
            Dress Materials for Women &amp; Kids • Verified Yardage
          </p>
          <a
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16 sm:h-20 gap-4">
          ...
          >
            <h1 className="font-serif text-xl sm:text-2xl font-bold tracking-tight text-neutral-900">
              DJ STYLE HUB
            </h1>
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-neutral-600">
            <button
              onClick={() => onSelectCategory('all')}
              className={`transition-colors ${activeCategory === 'all' ? 'text-neutral-900 font-bold' : 'hover:text-neutral-900'}`}
            >
              All Materials
            </button>
            <button
              onClick={() => onSelectCategory('women')}
              className={`transition-colors ${activeCategory === 'women' ? 'text-neutral-900 font-bold' : 'hover:text-neutral-900'}`}
            >
              Women's Collection
            </button>
            <button
              onClick={() => onSelectCategory('kids')}
              className={`transition-colors ${activeCategory === 'kids' ? 'text-neutral-900 font-bold' : 'hover:text-neutral-900'}`}
            >
              Kids' Collection
            </button>
            <button
        ...
        <div className="md:hidden border-t border-neutral-200 bg-white px-6 py-5 space-y-4">
          <div className="space-y-2">
            <button
              ...
              className={`block w-full text-left py-2 text-sm font-semibold ${activeCategory === 'all' ? 'text-neutral-900' : 'text-neutral-600'}`}
            >
              All Dress Materials
            </button>
            <button
              ...
              className={`block w-full text-left py-2 text-sm font-semibold ${activeCategory === 'women' ? 'text-neutral-900' : 'text-neutral-600'}`}
            >
              Women's Collection
            </button>
            <button
              ...
              className={`block w-full text-left py-2 text-sm font-semibold ${activeCategory === 'kids' ? 'text-neutral-900' : 'text-neutral-600'}`}
            >
              Kids' Collection
            </button>
            <button
